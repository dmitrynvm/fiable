...
def _pretty_quant(quant: str) -> str:
    """Keep the full scheme name (Q3_K_M, Q8_0, …)."""
    return str(quant).strip() if quant else quant
...
def plot_accuracy_vs_size(
    ...
    df = pd.DataFrame(rows)
    fig, ax = plt.subplots(figsize=(12, 7))
    series = list(df.groupby(["model", "task"]))
    for i, (key, part) in enumerate(series):
        model, task = key
        st = _SERIES[i % len(_SERIES)]
        ax.plot(
            part["size_gb"],
            part["acc"],
            marker=st["marker"],
    ...
    _style_axes(ax)
    return _finish_plot(df, output_path, "accuracy.png")
